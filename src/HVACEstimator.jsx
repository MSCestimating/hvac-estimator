    ...
    <div style={{ padding: "2rem", maxWidth: 1000, margin: "0 auto" }}>
      ...
      {counts && (
        <div style={{ background: "#f3f3f3", padding: "1rem", marginTop: "1rem", borderRadius: "8px" }}>
          ...
          <h5>📏 Rectangular Duct Sizes</h5>
          <ul>
            {Object.entries(counts.ductSizeLengthMap).map(([key, val]) => <li key={key}>{key}: {val} ft</li>)}
          </ul>
          <h5>⚖️ Rectangular Duct Weights</h5>
          <ul>
            {Object.entries(counts.ductSizeWeightMap).map(([key, val]) => <li key={key}>{key}: {val.toFixed(2)} lbs</li>)}
          </ul>
